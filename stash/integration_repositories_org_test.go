...
var _ = Describe("Stash Provider", func() {
	...
	It("should be possible to create a pr for an org repository", func() {
		...
		pr, err := orgRepo.PullRequests().Create(ctx, prTitleAndDesc, branchName, defaultBranch, prTitleAndDesc)
		Expect(err).ToNot(HaveOccurred())
		Expect(pr.Get().WebURL).ToNot(BeEmpty())
		Expect(pr.Get().SourceBranch).To(Equal(branchName))
		Expect(pr.Get().Title).To(Equal(prTitleAndDesc))
		Expect(pr.Get().Description).To(Equal(prTitleAndDesc))
	})
})
