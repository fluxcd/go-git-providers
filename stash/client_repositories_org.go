...
//nolint
func createRepository(ctx context.Context, c stashClient, ref gitprovider.RepositoryRef, projectName string, req gitprovider.RepositoryInfo, opts ...gitprovider.RepositoryCreateOption) (*Repository, error) {
	// First thing, validate and default the request to ensure a valid and fully-populated object
	// (to minimize any possible diffs between desired and actual state)
	if err := gitprovider.ValidateAndDefaultInfo(&req); err != nil {
		return nil, err
	...
	if *(o.AutoInit) {
		user, err := c.GetUserByID(ctx, repo.UserID)
		if err != nil {
			return nil, err
		}

		readmeContents := fmt.Sprintf("# %s\n%s", repo.Name, repo.Description)
		if err := createInitialCommit(user.Name, user.Email, readmeContents, *o.LicenseTemplate); err != nil {
...
func applyRepoCreateOptions(apiObj *Repository, opts gitprovider.RepositoryCreateOptions) {
}
...
func createInitialCommit(userName, userEmail, readmeContent string, license gitprovider.LicenseTemplate) error {
	...
	fmt.Fprintf(os.Stderr, "status: %s", status)

	// Commits the current staging area to the repository, with the new file
	// just created. We should provide the object.Signature of Author of the
	// commit Since version 5.0.1, we can omit the Author signature, being read
	// from the git config files.
	commit, err := w.Commit("inital Commit", &git.CommitOptions{
	...
	fmt.Fprintf(os.Stderr, "commit: %s", obj)
	return nil
}
