...
func (c *BranchClient) Create(ctx context.Context, branch, sha string) error {
	...
	_, err := c.c.GetCommit(ctx, c.ref.GetIdentity(), c.ref.GetRepository(), sha)
	if err != nil {
		return err
	}
	...
	return createBranch(ctx, c.clientContext, getRepoHTTPref(repo.Links.Clone), user.Name, user.EmailAddress, branch, sha)
}
