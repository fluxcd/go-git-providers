...
var _ = Describe("GitLab Provider", func() {
	...
	BeforeSuite(func() {
		gitlabToken := os.Getenv("MY_GITLAB_TOKEN")
		if len(gitlabToken) == 0 {
			b, err := ioutil.ReadFile(ghTokenFile)
			if token := string(b); err == nil && len(token) != 0 {
				gitlabToken = token
			} else {
				Fail("couldn't acquire MY_GITLAB_TOKEN env variable")
			}
		}

		if orgName := os.Getenv("MY_GITLAB_ORGANIZATION"); len(orgName) != 0 {
			testOrgName = orgName
		}

		if gitProviderUser := os.Getenv("MY_GITLAB_USER"); len(gitProviderUser) != 0 {
			testUserName = gitProviderUser
		}

		if teamName := os.Getenv("MY_GITLAB_TEAM_NAME"); len(teamName) != 0 {
