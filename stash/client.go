...
func newClient(c *gostash.Client, domain, token string, destructiveActions bool, log logr.Logger) *Client {
	sClient := &stashClientImpl{c, destructiveActions, log}
	ctx := &clientContext{sClient, domain, token, destructiveActions, log}
	return &Client{
		clientContext: ctx,
...
// SupportedDomain returns the domain endpoint for this client, e.g. "https://mystash.com:7990"
