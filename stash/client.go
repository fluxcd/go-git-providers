...
func newClient(c httpclient.ReqResp, domain string, sshDomain string, destructiveActions bool, log logr.Logger) *Client {
	sClient := &stashClientImpl{c, destructiveActions, log}
	ctx := &clientContext{sClient, domain, sshDomain, destructiveActions, log}
	return &Client{
		clientContext: ctx,
